...
namespace Cyggie.Plugins.WebSocket
{
    ...
    /// </summary>
    public class WSUnityClientService : Service<WSUnityClientServiceConfiguration>
    {
        /// <summary>
        ...
        public async Task BuildConnection(string url)
        {
            ...
            {
                Log.Debug($"Building a new connection while the previous one is active... disconnecting it.", nameof(WSUnityClientService));
                await Disconnect();
            }
        ...
        public void RegisterReceivingMethods(params WSClientMethod[] methods)
        {
            if (_conn == null)
            {
                Log.Error($"Failed to register receiving methods, connection has not been built yet, use BuildConnection() first.", nameof(WSUnityClientService));
                return;
            }

            foreach (WSClientMethod method in methods)
            {
                Log.Debug($"Registering method to WebSocket client: {method.MethodName} ({method.PrintParams()}).", nameof(WSUnityClientService));

                if (_methods.Any(x => x.MethodName == method.MethodName))
                {
                    Log.Error($"Method overloading on the client-side is not allowed: {method.MethodName}", nameof(WSUnityClientService));
                    continue;
                }
                ...
                {
                    Log.Error($"WS Client method callback is null for {method.MethodName}.", nameof(WSUnityClientService));
                };

                _conn.On(method.MethodName, method.FilteredTypes, callback);
            }

            Log.Debug($"Registered {methods.Length} callback(s).", nameof(WSUnityClientService));
        }
        ...
        public async Task Connect()
        {
            if (_conn == null)
            {
                Log.Error($"Failed to connect, connection has not been built yet, use BuildConnection() first.", nameof(WSUnityClientService));
                return;
            }

            try
            {
                Log.Debug($"Connecting to websocket server...", nameof(WSUnityClientService));

                await _conn.StartAsync();

                _reconnectionAttempts = 0;

                Log.Debug($"Connection successfully established.", nameof(WSUnityClientService));
                OnConnected?.Invoke();
            }
            catch (Exception ex)
            {
                Log.Error($"Failed to start connection, exception: {ex}.", nameof(WSUnityClientService));
        ...
        public async Task Disconnect()
        {
            if (_conn == null)
            {
                Log.Error($"Failed to disconnect, connection has not been built yet, use BuildConnection() first.", nameof(WSUnityClientService));
                return;
            }

            if (_conn.State == HubConnectionState.Disconnected)
            {
                Log.Error($"Failed to disconnect, connection is already disconnected.", nameof(WSUnityClientService));
                return;
            }

            Log.Debug($"Current state: {_conn.State}. Disconnecting...", nameof(WSUnityClientService));

            _expectDisconnection = true;
            await _conn.StopAsync();

            _methods.Clear();
            Log.Debug($"Connection disconnected successfully.", nameof(WSUnityClientService));
        }
        ...
        public async Task Call(string methodName, params object?[] args)
        {
            if (_conn == null)
            {
                Log.Error($"Failed in call for {methodName}, connection has not been built yet, use BuildConnection() first.", nameof(WSUnityClientService));
                return;
            }
            ...
            try
            {
                Log.Debug($"Calling {methodName} with {args.Length} arguments.", nameof(WSUnityClientService));

                await _conn.InvokeCoreAsync(methodName, args);
            }
            catch (Exception ex)
            {
                Log.Error($"Failed in call for {methodName} (args: {args.Length}), exception: {ex}.", nameof(WSUnityClientService));
            }
        }
        ...
        private async Task OnConnectionClosed(Exception? ex)
        {
            ...
            if (ex != null)
            {
                Log.Error($"Connection was unexpectedly closed due to an exception: {ex}.", nameof(WSUnityClientService));
            }
        ...
        private async Task CheckForReconnection()
        {
            ...
            switch (Configuration.ReconnectionType)
            {
                ...
                default:
                    Log.Error($"Unhandled switch-case type of {typeof(WSReconnectionType)}: {Configuration.ReconnectionType}.", nameof(WSUnityClientService));
                    break;
            }
        ...
        private async Task Reconnect(double delayInMs = 0)
        {
            ...
            if (delayInMs > 0)
            {
                Log.Error($"Failed to connect, retrying in {delayInMs}ms...", nameof(WSUnityClientService));
