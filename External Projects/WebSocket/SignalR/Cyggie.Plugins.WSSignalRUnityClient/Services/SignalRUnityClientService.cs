...
namespace Cyggie.Plugins.SignalR.Services
{
    ...
    /// </summary>
    public class SignalRUnityClientService : Service<SignalRUnityClientServiceConfiguration>
    {
        /// <summary>
        ...
        public async Task BuildConnection(string url, string? authToken = null)
        {
            if (_conn != null && _conn.State == HubConnectionState.Connected)
            {
                Log.Debug($"Building a new connection while the previous one is active... disconnecting it.", nameof(SignalRUnityClientService));
                await Disconnect();
            }
        ...
        public void RegisterReceivingMethods(params SignalRClientMethod[] methods)
        {
            if (_conn == null)
            {
                Log.Error($"Failed to register receiving methods, connection has not been built yet, use BuildConnection() first.", nameof(SignalRUnityClientService));
                return;
            }

            foreach (SignalRClientMethod method in methods)
            {
                Log.Debug($"Registering method to WebSocket client: {method.MethodName} ({method.PrintParams()}).", nameof(SignalRUnityClientService));

                if (_methods.Any(x => x.MethodName == method.MethodName))
                {
                    Log.Error($"Method overloading on the client-side is not allowed: {method.MethodName}", nameof(SignalRUnityClientService));
                    continue;
                }
                ...
                {
                    Log.Error($"WS Client method callback is null for {method.MethodName}.", nameof(SignalRUnityClientService));
                };

                _conn.On(method.MethodName, method.FilteredTypes, callback);
            }

            Log.Debug($"Registered {methods.Length} callback(s).", nameof(SignalRUnityClientService));
        }
        ...
        public async Task Connect()
        {
            if (_conn == null)
            {
                Log.Error($"Failed to connect, connection has not been built yet, use BuildConnection() first.", nameof(SignalRUnityClientService));
                return;
            }

            try
            {
                Log.Debug($"Connecting to websocket server...", nameof(SignalRUnityClientService));

                await _conn.StartAsync();

                _reconnectionAttempts = 0;

                Log.Debug($"Connection successfully established.", nameof(SignalRUnityClientService));
                OnConnected?.Invoke();
            }
            catch (Exception ex)
            {
                Log.Error($"Failed to start connection, exception: {ex}.", nameof(SignalRUnityClientService));
        ...
        public async Task Disconnect()
        {
            if (_conn == null)
            {
                Log.Error($"Failed to disconnect, connection has not been built yet, use BuildConnection() first.", nameof(SignalRUnityClientService));
                return;
            }

            if (_conn.State == HubConnectionState.Disconnected)
            {
                Log.Error($"Failed to disconnect, connection is already disconnected.", nameof(SignalRUnityClientService));
                return;
            }

            Log.Debug($"Current state: {_conn.State}. Disconnecting...", nameof(SignalRUnityClientService));

            _expectDisconnection = true;
            await _conn.StopAsync();

            _methods.Clear();
            Log.Debug($"Connection disconnected successfully.", nameof(SignalRUnityClientService));
        }
        ...
        public async Task Call(string methodName, params object?[] args)
        {
            if (_conn == null)
            {
                Log.Error($"Failed in call for {methodName}, connection has not been built yet, use BuildConnection() first.", nameof(SignalRUnityClientService));
                return;
            }
            ...
            try
            {
                Log.Debug($"Calling {methodName} with {args.Length} arguments.", nameof(SignalRUnityClientService));

                await _conn.InvokeCoreAsync(methodName, args);
            }
            catch (Exception ex)
            {
                Log.Error($"Failed in call for {methodName} (args: {args.Length}), exception: {ex}.", nameof(SignalRUnityClientService));
            }
        }
        ...
        private async Task OnConnectionClosed(Exception? ex)
        {
            ...
            if (ex != null)
            {
                Log.Error($"Connection was unexpectedly closed due to an exception: {ex}.", nameof(SignalRUnityClientService));
            }
        ...
        private async Task CheckForReconnection()
        {
            ...
            switch (Configuration.ReconnectionType)
            {
                ...
                default:
                    Log.Error($"Unhandled switch-case type of {typeof(SignalRReconnectionType)}: {Configuration.ReconnectionType}.", nameof(SignalRUnityClientService));
                    break;
            }
        ...
        private async Task Reconnect(double delayInMs = 0)
        {
            ...
            if (delayInMs > 0)
            {
                Log.Error($"Failed to connect, retrying in {delayInMs}ms...", nameof(SignalRUnityClientService));
